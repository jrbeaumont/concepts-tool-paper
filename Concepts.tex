...
\For {Signal $s$ in \textbf{System}}
  ...
  \For {Each \textbf{transition} $t$  \textbf{of} $s$}
    \If {$t$ is $rise$}
...
\For {Each \textbf{transition} $t$ in $transitions$}
  \If {$t$ is $rise$}
    \For {Each \textbf{effect transition} $e$ of $t$}
      \State \textbf{read-arc} (place $t$.signalName$1$, $e$)
    \EndFor
  \ElsIf{$t$ is $fall$}
    \For {Each \textbf{effect transition} $e$ for $t$}
...
For example, for \mintinline{haskell}{rise a ~> rise b} we connect the place after $a^{+}$, namely $a0$,
