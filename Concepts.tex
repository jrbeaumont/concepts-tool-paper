...
The abstract base of concepts, on which these asynchronous specifications of 
...
\begin{center}
{\mintinline{haskell}{rise a ~>} \mintinline{haskell}{rise c}}
\end{center}
...
\begin{center}
\mintinline{haskell}{rise a ~>} \mintinline{haskell}{rise c <>} \mintinline{haskell}{rise b ~>} \mintinline{haskell}{rise c}
\end{center}
...
\begin{center}
\mintinline{haskell}{inputs [a, b] <>} \mintinline{haskell}{outputs [c]} \mintinline{haskell}{<>} \mintinline{haskell}{internals [t]}
\end{center}
...
\begin{center}
   \mintinline{haskell}{initialise a 0 <>}\mintinline{haskell}{initialise x 1}
\end{center}
...
\begin{center}
\mintinline{haskell}{initialise0[a, b, c] <>} \mintinline{haskell}{initialise1[x,y]}
\end{center}
...
There can be multiple ways of representing a specification using concepts, however,
...
is at least one single cause transitions for each effect transition. If there were 
...
circuit a b c = operation <> interface 
             <> inits 
 where
  operation = cElement a b c <> environment
  environment = inverter c a 
             <> inverter c b
  interface = inputs[a, b] <> outputs[c] 
  inits = initialise0 [a, b, c]
\end{minted}
...
\begin{centering}
\vspace{-3mm}
\includegraphics[scale=0.41]{Images/workcraft_design_flow.png}
\par\end{centering}
\begin{centering}
